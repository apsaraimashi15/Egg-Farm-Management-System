...
exports.markAttendance = async (req, res) => {
  try {
    ...
    if (leave && status !== 'leave') {
      return res.status(400).json({ message: 'Cannot change status for approved leave days' });
    }
...
exports.getAttendanceByDate = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.findOne({ userId, date: parsedDate }).populate('markedBy', 'name email');
...
exports.getEmployeesMonthlySummary = async (req, res) => {
  try {
    ...
    const summaries = await Promise.all(employees.map(async (emp) => {
      ...
      const daysLeaved = attendances.filter(a => a.status === 'leave').length;
