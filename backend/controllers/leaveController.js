...
exports.getLeaveRequests = async (req, res) => {
  try {
    ...
    const leaveRequests = await Leave.find(query).populate(
      "userId",
      "name email"
    );
...
exports.updateLeaveRequest = async (req, res) => {
  try {
    ...
    if (leave.status !== "pending") {
      return res
        .status(403)
        .json({ message: "Cannot update approved or rejected leave requests" });
    }
...
exports.deleteLeaveRequest = async (req, res) => {
  try {
    ...
    if (leave.status !== "pending" && req.user.role !== "hrmanager") {
      return res
        .status(403)
        .json({ message: "Cannot delete approved or rejected leave requests" });
    }
