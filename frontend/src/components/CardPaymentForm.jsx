    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-slate-600">
            ...
            {
              Object.keys(cardDetails).filter(
                (k) => k !== "expiryMonth" && k !== "expiryYear"
              ).length
            }{" "}
            fields valid
          </span>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">
          ...
          {cardType && (
            <span className="ml-2 text-sm text-slate-500 capitalize">
              {cardType}
            </span>
          )}
        </label>
        <div className="relative">
          <input
            ...
            onBlur={handleFieldBlur}
            className={`w-full px-4 py-3 pr-12 border rounded-xl focus:outline-none focus:ring-2 transition-all duration-200 ${getFieldClasses(
              "cardNumber"
            )}`}
        ...
        {touchedFields.cardNumber && !fieldValidity.cardNumber && (
          <div className="mt-2 flex items-start space-x-2">
            ...
            <p className="text-sm text-red-600">
              {fieldErrors.cardNumber || "Please enter a valid card number"}
            </p>
          </div>
        )}
      ...
      <div>
        ...
        {touchedFields.cvv && !fieldValidity.cvv && cardDetails.cvv && (
          <div className="mt-2 flex items-start space-x-2">
            ...
            <p className="text-sm text-red-600">
              {fieldErrors.cvv ||
                `CVV must be ${cardType === "amex" ? "4" : "3"} digits`}
            </p>
          </div>
        )}
        <p className="mt-1 text-xs text-slate-500">
