...
const Checkout = () => {
    ...
    switch (name) {
      ...
      case 'phoneNumber': {
        ...
        if (!value) {
          ...
          errors.phoneNumber = 'Please enter a valid Sri Lankan phone number (+94712345678 or 0712345678)'
        } else {
      ...
      case 'cardholderName': {
        ...
        if (!value || value.trim().length < 2) {
          ...
          errors.cardholderName = 'Cardholder name must be less than 50 characters'
        } else if (!nameRegex.test(value)) {
          errors.cardholderName = 'Cardholder name can only contain letters, spaces, hyphens, apostrophes, and periods'
        } else {
      ...
      case 'accountNumber': {
        ...
        if (!value) {
          ...
          errors.accountNumber = 'Please enter a valid account number (8-20 digits)'
        } else {
      ...
      case 'mobilePaymentAccountHolderName': {
        if (!value || value.trim().length < 2) {
          errors.mobilePaymentAccountHolderName = 'Account holder name is required'
        } else {
      ...
      case 'mobilePaymentMobileNumber': {
        ...
        if (!value) {
          ...
          errors.mobilePaymentMobileNumber = 'Please enter a valid mobile number'
        } else {
  ...
  const handleCardValidationChange = (fieldName, isValid, errorMessage) => {
    ...
    if (isValid) {
      ...
      switch (fieldName) {
        case 'cardholderName':
          errors[fieldName] = 'Cardholder name must be at least 2 characters and contain only letters'
          break
        case 'cardNumber':
          errors[fieldName] = 'Please enter a valid card number'
          break
        case 'expiryDate':
          errors[fieldName] = 'Please select a valid expiry date (not in the past)'
          break
        case 'cvv':
    ...
    if (isValid) {
      ...
      switch (fieldName) {
        case 'fullName':
          ...
          break
        case 'phoneNumber':
          errors[fieldName] = 'Please enter a valid Sri Lankan phone number (+94712345678 or 0712345678)'
          break
        case 'street':
        ...
        case 'postalCode':
          ...
          break
        case 'nic':
          errors[fieldName] = 'Please enter a valid NIC number (9 digits + V/X or 12 digits)'
          break
        default:
          errors[fieldName] = 'This field is required'
      }
  ...
  const handleSubmitOrder = async (e) => {
    ...
    if (!address.phoneNumber || !/^(\+94|0)[0-9]{9}$/.test(address.phoneNumber)) {
      setError('Please enter a valid Sri Lankan phone number (e.g., +94712345678 or 0712345678)')
      return
    }
    ...
    if (paymentMethod === 'card') {
      ...
      if (!cardDetails.cardNumber || !isValidCardNumber(cardDetails.cardNumber.replace(/\s+/g, ''))) {
        setError('Please enter a valid card number')
        return
      }

      if (!cardDetails.expiryDate || !isValidExpiryDate(cardDetails.expiryDate)) {
    ...
    try {
      ...
      const purchaseData = {
        ...
        paymentMethod,
        notes: `Payment method: ${paymentMethod}${paymentMethod === 'mobile' ? ` (${mobilePayment.provider})` : ''}`
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-purple-50 flex">
        ...
        <div className="flex-1 lg:ml-64 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-500 mb-6">Add some products to your cart before checking out.</p>
            <button
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-purple-50">
      ...
      {/* Notification Toast */}
      {notification && (
        <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl shadow-lg flex items-center space-x-2 animate-fade-in ${
