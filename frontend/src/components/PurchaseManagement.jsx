  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/purchases/stats",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    try {
      ...
      try {
        ...
        if (logoResponse.ok) {
          ...
          pdf.setFontSize(18);
          pdf.setFont("helvetica", "bold");
          pdf.text("EGG FARM MANAGEMENT", pageWidth / 2, 17, {
            align: "center",
          });
      ...
      function addContent() {
        ...
        if (filteredPurchases.length > 0) {
          ...
          filteredPurchases.forEach((purchase, index) => {
            ...
            pdf.setFont("helvetica", "bold");
            const customerName =
              purchase.buyer.name.length > 15
                ? purchase.buyer.name.substring(0, 15) + "..."
                : purchase.buyer.name;
            ...
            pdf.setTextColor(100, 116, 139);
            const customerEmail =
              purchase.buyer.email.length > 15
                ? purchase.buyer.email.substring(0, 15) + "..."
            ...
            const itemsText = `${purchase.items.length} item${
            ...
            pdf.text(
              `LKR ${purchase.totalAmount?.toFixed(2) || "0.00"}`,
              totalX,
              yPosition + 2.5
            );
        ...
        // Save PDF
        const fileName = `egg-farm-purchases-${
      ...
      function addFooter(pdf, pageHeight) {
        ...
        pdf.setFont("helvetica", "normal");
        pdf.text(
          "Egg Farm Management System - Confidential Document",
          pageWidth / 2,
          footerY,
          { align: "center" }
        );
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-slate-900">
                      LKR {stats.totalRevenue?.toFixed(2) || "0.00"}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-100 text-sm">
                      Order #{selectedPurchase._id.slice(-8)}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="text-slate-900 font-semibold">
                        {selectedPurchase.items.length} item
                        {selectedPurchase.items.length !== 1 ? "s" : ""}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-100 rounded-2xl p-6 mb-6 border border-purple-200">
                  ...
                  <div className="bg-white/70 rounded-xl p-4 border border-purple-200">
                    <p className="text-slate-900 font-medium">
                      {selectedPurchase.address.fullName}
                    </p>
                    <p className="text-slate-700">
                      {selectedPurchase.address.street}
                    </p>
                    <p className="text-slate-700">
                      {selectedPurchase.address.city},{" "}
                      {selectedPurchase.address.district}
                    </p>
                    <p className="text-slate-700">
                      {selectedPurchase.address.province},{" "}
                      {selectedPurchase.address.postalCode}
                    </p>
                    <p className="text-slate-700">
                      {selectedPurchase.address.country}
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 mb-6">
                ...
                <div className="space-y-4">
                  {selectedPurchase.items.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl border border-slate-200 hover:shadow-md transition-shadow"
                    >
                      <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl flex items-center justify-center shadow-lg">
                          <span className="text-white text-lg">
                            {item.productType === "eggs"
                              ? "🥚"
                              : item.productType === "fertilizer"
                              ? "🌱"
                              : "🌾"}
                          </span>
                        </div>
                      ...
                      <div className="text-right">
                        <p className="text-xl font-bold text-emerald-600">
                          LKR {item.totalPrice?.toFixed(2) || "0.00"}
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-3xl font-bold">
                      LKR {selectedPurchase.totalAmount?.toFixed(2) || "0.00"}
                    </p>
                  </div>
                </div>
              </div>
