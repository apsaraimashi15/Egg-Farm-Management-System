...
const ReportManagement = () => {
  ...
  const generateCombinedPDF = async () => {
    try {
      ...
      if (pdfDateRange.start && pdfDateRange.end) {
        doc.text(`Period: ${pdfDateRange.start} to ${pdfDateRange.end}`, pageWidth / 2, y, { align: 'center' });
        y += 8;
      }
      ...
      const totalEggs = filteredEggs.reduce((sum, e) => sum + (Number(e.quantity) || 0), 0);
      const totalFertilizerCollected = filteredFertilizers.reduce((sum, f) => sum + (Number(f.quantityCollected) || 0), 0);
      const totalFertilizerUsed = filteredFertilizers.reduce((sum, f) => sum + (Number(f.usedQuantity) || 0), 0);
      const totalFertilizerRemaining = totalFertilizerCollected - totalFertilizerUsed;

      doc.text(`Total Egg Production Records: ${filteredEggs.length}`, margin, y);
      y += 6;
      doc.text(`Total Eggs Collected: ${totalEggs}`, margin, y);
      y += 6;
      doc.text(`Total Fertilizer Collected: ${totalFertilizerCollected}`, margin, y);
      y += 6;
      doc.text(`Total Fertilizer Used: ${totalFertilizerUsed}`, margin, y);
      y += 6;
      doc.text(`Total Fertilizer Remaining: ${totalFertilizerRemaining}`, margin, y);
      y += 10;
      ...
      filteredFertilizers.forEach((fert, idx) => {
        if (y > 280) { doc.addPage(); y = 20; }
        const remaining = (fert.quantityCollected || 0) - (fert.usedQuantity || 0);
        doc.text(String(idx + 1), margin, y);
      ...
      // Save the PDF
      const fileName = pdfDateRange.start && pdfDateRange.end
  ...
  const generateEggPDF = async () => {
    try {
      ...
      if (pdfDateRange.start && pdfDateRange.end) {
        doc.text(`Period: ${pdfDateRange.start} to ${pdfDateRange.end}`, margin, y);
        y += 6;
      }

      const totalQty = filteredEggs.reduce((a, b) => a + (Number(b.quantity) || 0), 0);
      doc.text(`Total Records: ${filteredEggs.length}   |   Total Quantity: ${totalQty}`, margin, y);
      y += 8;
  ...
  const generateFertilizerPDF = async () => {
    try {
      ...
      const totalCollected = fertilizerData.reduce((a, b) => a + (Number(b.quantityCollected) || 0), 0);
      const totalUsed = fertilizerData.reduce((a, b) => a + (Number(b.usedQuantity) || 0), 0);
      const totalRemaining = totalCollected - totalUsed;
      
      doc.text(`Records: ${fertilizerData.length}   |   Collected: ${totalCollected}   |   Used: ${totalUsed}   |   Remaining: ${totalRemaining}`, margin, y);
      y += 8;
      ...
      doc.save(`fertilizer-report-${new Date().toISOString().slice(0, 10)}.pdf`);
      setShowPdfModal(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50 flex">
      ...
      <div className="flex-1">
        <div className="lg:hidden">
          <div className="flex items-center justify-between bg-white/80 backdrop-blur-sm px-4 py-3 shadow-sm border-b border-white/20">
            ...
            <h1 className="text-lg font-semibold text-slate-800">Reports Management</h1>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-800 rounded-2xl p-8 mb-8 text-white">
              <div className="relative z-10">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                  <div>
                    <h1 className="text-3xl font-bold tracking-tight mb-1">Report Management</h1>
                    <p className="text-emerald-100 text-lg">Generate comprehensive PDF reports for egg production and fertilizer tracking</p>
                  </div>
                  <div>
                    <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm text-slate-600 font-medium">Combined Report</p>
                    <p className="text-lg font-bold text-slate-900">Eggs + Fertilizer</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center">
                  <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center mr-4">
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c1.5 0 3 .5 4 1.5M8 16a6 6 0 118 0" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-sm text-slate-600 font-medium">Egg Production</p>
                    <p className="text-lg font-bold text-slate-900">Daily Records</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm text-slate-600 font-medium">Fertilizer</p>
                    <p className="text-lg font-bold text-slate-900">Collection Data</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {showPdfModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm" onClick={() => !pdfLoading && setShowPdfModal(false)}>
          <div className="bg-white rounded-2xl p-6 m-4 max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-900">Generate PDF Report</h3>
                <p className="text-sm text-slate-600">Choose report type and date range</p>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-2">Leave empty to include all records</p>
              </div>

              <div className="border-t border-slate-200 pt-4">
                <label className="block text-sm font-medium text-slate-700 mb-3">Select Report Type</label>
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-emerald-100">Eggs + Fertilizer</p>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-blue-100">Daily egg records</p>
                      </div>
                    </div>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-amber-100">Collection records</p>
                      </div>
                    </div>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
