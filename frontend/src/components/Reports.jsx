  ...
  const fetchReportData = async () => {
    try {
      ...
      const leaveResponse = await axios.get(
        `http://localhost:5000/api/leaves?month=${month}&year=${year}`,
        {
      ...
      const attendanceResponse = await axios.get(
        `http://localhost:5000/api/attendance/summary?month=${month}&year=${year}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const generatePDF = async (type) => {
    try {
      ...
      const addContent = () => {
        ...
        if (type === "leave") {
          pdf.text(
            `Total Leave Requests: ${reportData.leaveRequests.length}`,
            margin,
            yPosition
          );
          yPosition += 15;
          ...
          pdf.text(
            `Total Employees: ${reportData.payroll.length}`,
            margin,
            yPosition
          );
          yPosition += 15;
          ...
          pdf.text(
            `Total Employees: ${reportData.attendance.length}`,
            margin,
            yPosition
          );
          yPosition += 15;
                      ...
                      <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center mr-4">
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M9 17v-6h6v6m-6 0h6m-9 4h12a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
                          />
                        </svg>
                      </div>
                      <div>
                        ...
                        <p className="text-emerald-100 text-lg">
                          Generate and review workflow reports
                        </p>
                      </div>
                    </div>
                    <p className="text-slate-300 max-w-2xl">
                      View and download reports for leave requests, payroll, and
                      attendance for a selected month and year.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-8">
              <div className="px-6 py-4 border-b border-slate-200">
                ...
                <p className="text-sm text-slate-600 mt-1">
                  Select a month and year to generate reports
                </p>
              </div>
              <div className="p-6">
